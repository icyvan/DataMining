...
Вероятности появления объектов каждого из классов ![](https://latex.codecogs.com/gif.latex?P_{y}=P(y)) называется априорными вероятностями классов(как думали, что объекты распределены).

Условная вероятность ![](https://latex.codecogs.com/gif.latex?P(y|x)) наз апостериорной вероятностью классов *y* для объекта *x*(известно, после получения результата).
...
форму эллипсоидов с центром µ и осями, параллельными линиям координа
